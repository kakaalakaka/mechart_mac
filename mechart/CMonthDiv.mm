...
{
	m_calendar = 0;
	//vector<CMonthButtonMe*>::iterator sIter = m_monthButtons.begin();
	//for(; sIter != m_monthButtons.end(); ++sIter)
	//{
	//	delete *sIter;
	//	*sIter = 0;
	//}
	...
	//vector<CMonthButtonMe*>::iterator sIterAm = m_monthButtons_am.begin();
	//for(; sIterAm != m_monthButtons_am.end(); ++sIterAm)
	//{
	//	delete *sIterAm;
	//	*sIterAm = 0;
	//}
...
void CMonthDivMe::OnClick(const POINT& mp, MouseButtonsA button, int clicks, int delta)
{
	int count = (int)m_monthButtons.size();
	for (int i = 0; i < count; i++)
	{
