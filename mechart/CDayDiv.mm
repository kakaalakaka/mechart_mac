...
{
	m_calendar = 0;
	//vector<CDayButtonMe*>::iterator sIter = m_dayButtons.begin();
	//for(; sIter != m_dayButtons.end(); ++sIter)
	//{
	//	delete *sIter;
	//	*sIter = 0;
	//}
	...
	//vector<CDayButtonMe*>::iterator sIterAm = m_dayButtons_am.begin();
	//for(; sIterAm != m_dayButtons_am.end(); ++sIterAm)
	//{
	//	delete *sIterAm;
	//	*sIterAm = 0;
	//}
...
void CDayDivMe::OnClick(const POINT& mp, MouseButtonsA button, int clicks, int delta)
{
	int count = (int)m_dayButtons.size();
	for (int i = 0; i < count; i++)
	{
...
void CDayDivMe::OnResetDiv(int state)
{
	if (m_calendar)
	{
		...
		int x = 0;
		int heightHeadDiv = m_calendar->GetHeadDiv()->GetHeight();//v68
		int height = heightHeadDiv;//v67
		int width = m_calendar->GetWidth();//v66
		int dateDivHeight = m_calendar->GetHeight();//v65
		int timeDivHeight = m_calendar->GetTimeDiv()->GetHeight();//v7
