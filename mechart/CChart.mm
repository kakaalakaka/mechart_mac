...
namespace MeLib
{
	...
	void CChartMe::CheckToolTip()
    {
		...
		double nowTime = CMathLibMe::M129(1900 + ltm->tm_year, 1 + ltm->tm_mon, ltm->tm_mday,
	...
	void CChartMe::OnPaintBar(CPaintMe *paint, CDivMe *div, BarShape *bs)
	{
	...
	void CChartMe::OnMouseDown(const POINT& mp, MouseButtonsA button, int clicks, int delta)
	{
	...
	void CChartMe::OnMouseMove(const POINT& mp, MouseButtonsA button, int clicks, int delta)
	{
		...
		if (mp.x != m_lastMouseMovePoint.x || mp.y != m_lastMouseMovePoint.y)
        {
			...
			for(; sIter != divsCopy.end(); ++sIter)
			{
                ...
                if (resize && button == MouseButtonsA_Left)
                {
            ...
            time_t now = time(0);
            tm *ltm = localtime(&now);
            m_lastMouseMoveTime = CMathLibMe::M129(1900 + ltm->tm_year, 1 + ltm->tm_mon, ltm->tm_mday,
                                            ltm->tm_hour, ltm->tm_min, ltm->tm_sec, 0);
//            SYSTEMTIME t; 
//            GetLocalTime(&t); 
//            m_lastMouseMoveTime = CMathLibMe::M129(t.wYear,t.wMonth,t.wDay,t.wHour,t.wMinute,t.wSecond,t.wMilliseconds);
            if (m_movingPlot && button == MouseButtonsA_Left)
            {
	...
	void CChartMe::OnMouseUp(const POINT& mp, MouseButtonsA button, int clicks, int delta)
	{
		...
		if (m_hResizeType == 0 && m_userResizeDiv == 0 && button == MouseButtonsA_Left && m_canMoveShape && selectedShape)
        {
	...
	void CChartMe::OnMouseWheel(const POINT& mp, MouseButtonsA button, int clicks, int delta)
	{
