...
CursorsA CNativeBaseMe::GetCursor()
{
	if (m_host)
	{
...
void CNativeBaseMe::SetCursor( CursorsA cursor )
{
	if (m_host)
	{
...
void CNativeBaseMe::OnDoubleClick( MouseButtonsA button, int clicks, int delta )
{
	CControlMe *focusedControl = GetFocusedControl();
	if (focusedControl && IsPaintEnabled(focusedControl))
	{
...
void CNativeBaseMe::OnMouseDown( MouseButtonsA button, int clicks, int delta )
{
	m_draggingControl = 0;
	m_mouseDownControl = 0;
	POINT mousePoint = GetMousePoint();
...
void CNativeBaseMe::OnMouseLeave( MouseButtonsA button, int clicks, int delta )
{
...
void CNativeBaseMe::OnMouseMove( MouseButtonsA button, int clicks, int delta )
{
	POINT mousePoint = GetMousePoint();
	if (m_mouseDownControl)
	{
...
void CNativeBaseMe::OnMouseUp( MouseButtonsA button, int clicks, int delta )
{
	POINT mousePoint = GetMousePoint();
	vector<CControlMe*> sortedControls;
	GetSortedControls(0, &sortedControls);
...
void CNativeBaseMe::OnMouseWheel( MouseButtonsA button, int clicks, int delta )
{
	CControlMe *focusedControl = GetFocusedControl();
	if (focusedControl && IsPaintEnabled(focusedControl))
	{
...
bool CNativeBaseMe::OnPreviewsMouseEvent( int eventID, CControlMe *control, const POINT& mp, MouseButtonsA button, int clicks, int delta )
{
...
//TODO
void CNativeBaseMe::OnTouchBegin(vector<CTouch> *touches)
{
    int touchesSize = (int)touches->size();
    m_draggingControl = 0;
    m_mouseDownControl = 0;
    POINT mousePoint = GetMousePoint();
    m_mouseDownPoint = mousePoint;
    ...
    if (control)
    {
        ...
        if (IsPaintEnabled(control))
        {
            ...
            CControlMe *focusedControl = GetFocusedControl();
            m_mouseDownControl = control;
            m_mouseDownControl->OnTouchBegin(touches);
            if(touchesSize == 1)
            {
                if (focusedControl == GetFocusedControl())
                {
                ...
                if (!OnPreviewsMouseEvent(EVENTID::MOUSEDOWN, m_mouseDownControl, mousePoint, MouseButtonsA_Left, 1, 0))
                {
                    if (m_mouseDownControl)
                    {
                        m_mouseDownControl->OnMouseDown(mp, MouseButtonsA_Left, 1, 0);
                        if (m_mouseDownControl)
                        {
                            m_mouseDownControl->OnDragReady(&m_dragStartOffset);
                        }
...
void CNativeBaseMe::OnTouchCancel(vector<CTouch> *touches)
{
    m_draggingControl = 0;
    m_mouseDownControl = 0;
    m_mouseMoveControl = 0;
...
void CNativeBaseMe::OnTouchEnd(vector<CTouch> *touches)
{
    int touchesSize = (int)touches->size();
    POINT mousePoint = GetMousePoint();
    vector<CControlMe*> sortedControls;
    ...
    if (m_mouseDownControl)
    {
        m_mouseDownControl->OnTouchEnd(touches);
        CControlMe *control = m_mouseDownControl;
        if (OnPreviewsMouseEvent(EVENTID::MOUSEUP, control, mousePoint, MouseButtonsA_Left, 1, 0))
        {
            m_mouseDownControl = 0;
            return;
        }
        ...
        if (m_mouseDownControl)
        {
            if (touchesSize == 1)
            {
                ...
                if (la2 && la2 == control)
                {
                    control->OnClick(mp, MouseButtonsA_Left, 1, 0);
                }
                else
                {
                    m_mouseMoveControl = 0;
                ...
                if (m_mouseDownControl != 0)
                {
                    control = m_mouseDownControl;
                    m_mouseDownControl = 0;
                    control->OnMouseUp(mp, MouseButtonsA_Left, 1, 0);
                }
            }
        }
    }
    else if (m_draggingControl)
    {
        ...
        CControlMe *la3 = m_draggingControl;
        m_draggingControl = 0;
        if (OnPreviewsMouseEvent(EVENTID::MOUSEUP, la3, mousePoint, MouseButtonsA_Left, 1, 0))
        {
            return;
        }
        la3->OnMouseUp(point3, MouseButtonsA_Left, 1, 0);
        la3->OnDragEnd();
        CControlMe *parent = la3->GetParent();
        if (parent)
        {
...
void CNativeBaseMe::OnTouchMove(vector<CTouch> *touches)
{
    int touchesSize = (int)touches->size();
    POINT mousePoint = GetMousePoint();
    if (m_mouseDownControl)
    {
        if (!OnPreviewsMouseEvent(EVENTID::MOUSEMOVE, m_mouseDownControl, mousePoint, MouseButtonsA_Left, 1, 0))
        {
            POINT mp = {mousePoint.x - ClientX(m_mouseDownControl), mousePoint.y - ClientY(m_mouseDownControl)};
            m_mouseDownControl->OnMouseMove(mp, MouseButtonsA_Left, 1, 0);
            SetCursor(m_mouseDownControl);
            if (((m_mouseDownControl->AllowDrag())
                 && ((abs((int) (mousePoint.x - m_mouseDownPoint.x)) > m_dragStartOffset.x)
                     || (abs((int) (mousePoint.y - m_mouseDownPoint.y)) > m_dragStartOffset.y)))
                && m_mouseDownControl->OnDragBegin())
            {
    ...
    else
    {
        ...
        GetSortedControls(0, &sortedControls);
        CControlMe *control = FindControl(mousePoint, &sortedControls);
        if(control && OnPreviewsMouseEvent(EVENTID::MOUSEMOVE, control, mousePoint, MouseButtonsA_Left, 1, 0))
        {
            return;
        }
        sortedControls.clear();
        if (m_mouseMoveControl != control)
        {
            if ((m_mouseMoveControl && IsPaintEnabled(m_mouseMoveControl)) && !m_mouseDownControl)
            {
                ...
                CControlMe *la5 = m_mouseMoveControl;
                m_mouseMoveControl = control;
                la5->OnMouseLeave(point3, MouseButtonsA_Left, 1, 0);
            }
            if ((control && IsPaintEnabled(control)) && !m_mouseDownControl)
            {
                POINT point4 = {mousePoint.x - ClientX(control), mousePoint.y - ClientY(control)};
                m_mouseMoveControl = control;
                control->OnMouseEnter(point4, MouseButtonsA_Left, 1, 0);
                control->OnMouseMove(point4, MouseButtonsA_Left, 1, 0);
                SetCursor(control);
            }
        }
        else if (control && IsPaintEnabled(control))
        {
            POINT point5 = {mousePoint.x - ClientX(control), mousePoint.y - ClientY(control)};
            m_mouseMoveControl = control;
            control->OnMouseMove(point5, MouseButtonsA_Left, 1, 0);
            SetCursor(control);
        }
    }
}
...
void CNativeBaseMe::SetAnchor( vector<CControlMe*> *controls, SIZE oldSize )
{
	if ((oldSize.cx != 0) && (oldSize.cy != 0))
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			//CMathLibMe::M132(&bounds, &parentSize, &oldSize, anchor.left, anchor.top, anchor.right, anchor.bottom);
