...
void CTextBoxMe::CursorDown()
{
	...
	if (host->IsKeyPress(0x10))//Keys.ShiftKey
	{
...
{
	if (GetNative()->GetHost()->IsKeyPress(0x10))//Keys.ShiftKey
	{
...
HorizontalAlignA CTextBoxMe::GetTextAlign()
{
	return m_textAlign;
}

void CTextBoxMe::SetTextAlign( HorizontalAlignA textAlign )
{
...
void CTextBoxMe::OnChar( wchar_t ch )
{
	...
	{
		DivMe::OnChar(ch);
		if (!GetNative()->GetHost()->IsKeyPress(0x11))//Keys.ControlKey
		{
...
void CTextBoxMe::OnKeyDown( char key )
{
	...
	else if (key >= '#' && key <= '(')
	{
		if (key == '&' || key == '(')
		{
			DivMe::CallKeyEvents(0x12, key);//Keys.Menu
...
{
	CControlMe::OnKeyUp(key);//Keys.ShiftKey
...
void CTextBoxMe::OnMouseDown( const POINT& mp, MouseButtonsA button, int clicks, int delta )
{
...
void CTextBoxMe::OnMouseMove( const POINT& mp, MouseButtonsA button, int clicks, int delta )
{
...
void CTextBoxMe::OnMouseUp( const POINT& mp, MouseButtonsA button, int clicks, int delta )
{
	m_isMouseDown = false;
    ...
    if ((width > 0) && (height > 0))
    {
        ...
        for (int i = 0; i < lineCount; i++)
        {
            ...
            for (int n = line.m_start; n <= line.m_end; n++)
            {
				...
				RECT rRect = {(int)rangeRect.left,
                    (int)rangeRect.top + ((m_lineHeight - (int)m_wordsSize[n].cy) / 2),
                    (int)rangeRect.right,
                    (int)rangeRect.top + ((m_lineHeight + (int)m_wordsSize[n].cy) / 2)};
        ...
        if (selectedRangesSize > 0)
        {
            ...
            for (int i = 0; i < selectedRangesSize; i++)
            {
                ...
                if (newLine || i == selectedRangesSize - 1)
                {
					...
					RECT unionRect = {(int)sRect.left, (int)sRect.top, (int)eRect.right + 1, (int)sRect.bottom + 1};
                    if (newLine)
                    {
        ...
        if (((IsFocused() && !m_readOnly) && (m_selectionLength == 0)) && (m_isKeyDown || m_showCursor))
        {
            ...
            {
				RECT cRect = {(cursorX - scrollH - 1),  (cursorY - scrollV),
					(cursorX - scrollH + 1), cursorY + (int)tSize.cy - scrollV};
