...
void CTimeDivMe::OnLoad()
{
	if (m_calendar)
	{
		...
		if (!m_spinHour)
		{
			...
			m_spinHour->RegisterEvent((void *)SelectedTimeChanged, EVENTID::SELECTEDTIMECHANGED, this);
        }
		if (!m_spinMinute)
		{
			...
			m_spinMinute->RegisterEvent((void *)SelectedTimeChanged, EVENTID::SELECTEDTIMECHANGED, this);
		}
		if (!m_spinSecond)
		{
			...
			m_spinSecond->RegisterEvent((void *)SelectedTimeChanged, EVENTID::SELECTEDTIMECHANGED, this);
		}
	}
}
void CTimeDivMe::OnPaint(CPaintMe *paint, const RECT& clipRect)
{
    ...
    if (m_height > 0)
    {
        ...
        SIZE size = paint->TextSize(L"", font);
        RECT rectHour;
        ...
        paint->DrawText(L"", paintingForeColor, font, rectHour);
        size = paint->TextSize(L"", font);
        RECT rectMinute;
        ...
        paint->DrawText(L"", paintingForeColor, font, rectMinute);
        size = paint->TextSize(L"", font);
        RECT rectSecond;
        ...
        paint->DrawText(L"", paintingForeColor, font, rectSecond);
    }
}
