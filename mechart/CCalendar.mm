...
int CCalendarMe::DayOfWeek(int y, int m, int d)
{
	...
	//struct tm localMt;
	//localMt.tm_year = y - 1900;
	//localMt.tm_mon = m;
	//localMt.tm_mday = d;
	//time_t t = mktime(&localMt);
	//struct tm *localMt1 = localtime(&t);
	//return localMt1->tm_wday;
}
...
CMonthMe* CCalendarMe::GetLastMonth(int year, int month)
{
	...
	//;m_years.GetYear(nowYear).Months[lastMonth];
}
...
CMonthMe* CCalendarMe::GetNextMonth(int year, int month)
{
	...
	//m_years.GetYear(nowYear).Months[nextMonth];
}
...
void CCalendarMe::GetProperty(const String& name, String *value, String *type)
{
	if (name == L"selectedday")
	{
		*type = L"string";
		...
		swprintf(calendarValue, 99, L"%d-%02d-%02d", m_year, m_month, m_selectedDay->GetDay());
...
void CCalendarMe::OnClick(const POINT& mp, MouseButtonsA button, int clicks, int delta)
{
...
//        GetLocalTime(&SystemTime);
		...
		//SelectedDay = m_years.GetYear(m_year).Months[m_month].Days[now.Day];
	}
}
...
void CCalendarMe::SetProperty(const String& name, const String& value)
{
	...
	{ 
		...
		//SetSelectedDay(m_years.GetYear(m_year).Months[m_month].Days[time.Day];
	}
	else if (name == L"useanimation")
	{
