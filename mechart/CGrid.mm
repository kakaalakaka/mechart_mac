...
namespace MeLib
{
	...
	void CGridMe::CallCellMouseEvents(int eventID, CGridCellMe *cell, const POINT& mp, MouseButtonsA button, int clicks, int delta)
	{
		if(m_events.size() > 0)
		{
			map<int, vector<void*>*>::iterator sIter = m_events.find(eventID);
	...
	void CGridMe::MouseEvent(const POINT& mp, MouseButtonsA button, int clicks, int delta, int state)
	{
	...
	CGridMe::~CGridMe()
	{
		...
		if(m_editTextBox)
		{
			...
			{
				m_editTextBox->UnRegisterEvent((void *)m_editTextBoxLostFocusEvent, EVENTID::LOSTFOCUS);
				m_editTextBoxLostFocusEvent = 0;
			}
			if(m_editTextBoxKeyDownEvent)
			{
				m_editTextBox->UnRegisterEvent((void *)m_editTextBoxKeyDownEvent, EVENTID::KEYDOWN);
  ...
  //      int floor = GetHeight() - cell;
		//RECT bounds = row->GetBounds();
		//return CMathLibMe::M136(&bounds, row->GetHeight(), scrollV, visiblePercent, cell, floor);
	}
	...
	void CGridMe::OnCellClick(CGridCellMe *cell, const POINT& mp, MouseButtonsA button, int clicks, int delta)
	{
	...
	void CGridMe::OnCellEditBegin(CGridCellMe *cell)
	{
		...
		if (!m_editTextBox)
        {
			...
			m_editTextBox->RegisterEvent((void *)m_editTextBoxLostFocusEvent, EVENTID::LOSTFOCUS, this);
			m_editTextBox->RegisterEvent((void *)m_editTextBoxKeyDownEvent, EVENTID::KEYDOWN, this);
	...
	void CGridMe::OnCellMouseDown(CGridCellMe *cell, const POINT& mp, MouseButtonsA button, int clicks, int delta)
	{
		CallCellMouseEvents(EVENTID::GRIDCELLMOUSEDOWN, cell, mp, button, clicks, delta);
	}

	void CGridMe::OnCellMouseEnter(CGridCellMe *cell, const POINT& mp, MouseButtonsA button, int clicks, int delta)
	{
        ...
        {
            m_native->GetHost()->ShowToolTip(cell->GetPaintText(), m_native->GetMousePoint());
        }
	}

	void CGridMe::OnCellMouseLeave(CGridCellMe *cell, const POINT& mp, MouseButtonsA button, int clicks, int delta)
	{
		CallCellMouseEvents(EVENTID::GRIDCELLMOUSELEAVE, cell, mp, button, clicks, delta);
	}

	void CGridMe::OnCellMouseMove(CGridCellMe *cell, const POINT& mp, MouseButtonsA button, int clicks, int delta)
	{
		CallCellMouseEvents(EVENTID::GRIDCELLMOUSEMOVE, cell, mp, button, clicks, delta);
	}

	void CGridMe::OnCellMouseUp(CGridCellMe *cell, const POINT& mp, MouseButtonsA button, int clicks, int delta)
	{
	...
	void CGridMe::OnMouseDown(const POINT& mp, MouseButtonsA button, int clicks, int delta)
	{
		DivMe::OnMouseDown(mp, button, clicks, delta);
		MouseEvent(mp, button, clicks, delta, 1);
	}

	void CGridMe::OnMouseLeave(const POINT& mp, MouseButtonsA button, int clicks, int delta)
	{
	...
	void CGridMe::OnMouseMove(const POINT& mp, MouseButtonsA button, int clicks, int delta)
	{
		DivMe::OnMouseMove(mp, button, clicks, delta);
		MouseEvent(mp, button, clicks, delta, 0);
	}

	void CGridMe::OnMouseUp(const POINT& mp, MouseButtonsA button, int clicks, int delta)
	{
	...
	void CGridMe::SortColumn(CGridMe *grid, CGridColumnMe *column, GridColumnSortMode sortMode)
	{
		...
		//grid->GetSort()->SortColumn(grid, column, sortMode);
	}
